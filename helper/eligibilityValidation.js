...
const transformRisk = (range = 0) => {
  ...
  if (regularRange.includes(range)) return IStatus.Regular;
  if (range >= 3) return IStatus.Responsible;
...
const calculateAgeAndIncomeRange = (range = 0, riskCriteria) => {
  ...
  if (age < 30) range -= 2;
  else if (age >= 30 && age <= 40) range -= 1;

  if (income > 200000) range -= 1;
  return range;
};
...
const autoInsuranceEligibility = (range = 0, riskCriteria) => {
  ...
  if (!vehicle) return IStatus.Ineligible;
  const yearDifference = Math.abs(currentYear - vehicle.year);
  if (yearDifference <= 5) range += 1;
...
const disabilityInsuranceEligibility = (range = 0, riskCriteria) => {
  ...
  if (!income || age > 60) return IStatus.Ineligible;
  if (house.ownership_status === IOwnershipStatus.Mortgaged) range += 1;
  if (dependents) range += 1;
  if (maritalStatus === IMaritalStatus.Married) range -= 1;
  return transformRisk(range);
};

const homeInsuranceEligibility = (range = 0, riskCriteria) => {
  const { house } = riskCriteria;
  if (!house) return IStatus.Ineligible;
  if (house.ownership_status === IOwnershipStatus.Mortgaged) range += 1;
  return transformRisk(range);
};
